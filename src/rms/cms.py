...
CMS_SALT=bytes([77,197,101,206,190,249,93,200,51,243,93,237,71,94,239,138,68,108,70,185,225,137,217,16,51,122,193,48,194,195,198,175,172,169,70,84,61,62,104,186,114,52,61,168,66,129,192,208,187,249,232,193,41,113,41,45,240,16,29,228,208,228,61,20,0])

def challenge_response(challenge:str,password:str)->int:
    d=hashlib.md5(challenge.strip().encode('ascii')+password.strip().upper().encode('ascii')+CMS_SALT).digest()
    return ((d[3]&0x3F)<<24)|(d[2]<<16)|(d[1]<<8)|d[0]
    ...
    async def connect(self):
        ...
        while not self.ready:
            ...
            if 'Callsign :' in text:
                ...
                if line:
                    response=f'{challenge_response(line[5:],self.password):010d}'[2:]
